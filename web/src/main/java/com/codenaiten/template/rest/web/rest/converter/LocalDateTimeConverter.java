...
/**
 * Convierte un {@link String} a un objeto de tipo {@link LocalDateTime}.
 */
@Slf4j
@Component
...
// ------------------------------------------------------------------------------------------------------------------ \\
    ...
    public LocalDateTime convert( final String source ){
        final DateTimeFormatter formatter = DateTimeFormatter.ofPattern( this.temporalFormatProperties.getDateTime() );
        return Optional.ofNullable( source ).map( value -> LocalDateTime.parse( value, formatter )).orElse( null );
    }
